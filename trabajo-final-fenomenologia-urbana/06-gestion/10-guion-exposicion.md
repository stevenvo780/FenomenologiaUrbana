...
- señalar que el proyecto está en `0.2.0-baseline` con trazabilidad explícita;
- enfatizar que la UI ya distingue entre `documented`, `proxy` y `pending`, y además muestra cierre operativo.
...
- DANE geovisor fino, MEData uso del suelo y MEData equipamientos siguen bloqueados por `403`; el fallback DANE municipal/microdatos ya está integrado;
